...
<html>
   <head>
      <title>Sales Invoice | Superstore
      </title>
      <meta charset="UTF-8">
      ...
      <?php
         require '../config/dbConnection.php';
            $sql2 = "SELECT product_id, unit_price, discount, hst FROM product_master";
            $result2 = mysqli_query($link,$sql2);
            
            $sql = "SELECT customer_id, first_name, last_name from customer_master";
      ...
      <script type="text/javascript">
         ...
         function updatePrice() {
            ...
            var elements = document.getElementById("salesInvoice").elements;
            var temp = 0;
            for (var i = 0, element; element = elements[i++];) {
               ...
               if (typeof element !== 'undefined' && element.name.includes("discount")){
                  var temp = i-3;
                  var prod_id = elements[temp].value;
                  for (var j in products) {
                     if (products[j]['product_id'] == prod_id){
                        element.value = products[j]['discount']+'%';
                     }
                  }
               }
         ...
         function calcTotal() {
            ...
            for (var i = 0, element; element = elements[i++];) {
               ...
               if (typeof element !== 'undefined' && element.name.includes("delete")){
                  ...
                  var disc = parseInt(elements[temp-5].value);
                  var up = parseFloat(elements[temp-6].value); 

                  var discountedPrice = ((100-disc)/100)*up;
                  var taxableTotal = ((100+hst)/100)*discountedPrice*qty;
                  elements[temp-2].value = (taxableTotal).toFixed(2);
                 
                  total += (qty * up);
                  discountedTotal += (up*disc/100)*qty;
                  totalTax += (discountedPrice*hst/100)*qty;
                  finalAmount += taxableTotal;
               }
            }
            document.getElementById("total").value = "$"+(total).toFixed(2);
            document.getElementById("tax").value = "$"+(totalTax).toFixed(2);
            document.getElementById("finalDisc").value = "$"+(discountedTotal).toFixed(2);
               ...
               <div class="col-lg-12 col-xl-11">
                  <div class="container mt-3">
                     <div class="card" style="border-radius: 25px;">
                        <div class="card-body">
                           <p class="text-center h1 fw-bold mb-5 mx-1 mx-md-4 mt-4">Create Sales Invoice <i class="fa-solid fa-cart-shopping mx-1"></i></p>
                           <form name="salesInvoice" action="../controllers/saveSalesDetails.php" method="post" id="salesInvoice">
                           <div class="container">
                              <div class="row clearfix">
                                 <div class="col-md-12 table-responsive">
                                    <div class="row mx-1">
                                       ...
                                       Select Customer : <font color="red">*</font>
                                       </div>
                                       <div class="col-md-3">
                                       ...
                                       <tbody>
                                          <tr id='addr0' data-id="0" class="hidden">
                                             ...
                                             <td data-name="unitPrice">
                                                <input type="number" id="unitPrice" name="unitPrice[]" class="form-control" name="unitPrice" readOnly="" />
                                             </td>
                                             <td data-name="discount">
                                                <input type="tax" id="discount" name="discount[]" class="form-control" name="discount" readOnly="" />
                                             </td>
                                             <td data-name="hst">
                                                <input type="tax" id="hst" name="hst[]" class="form-control" readOnly=""/>
                                             </td>
                                             <td data-name="quantity">
                                                <input type="number" id="quantity" name="quantity[]" min="1" class="form-control" onchange="calcTotal()"/>
                                             </td>
                                             <td data-name="itemTotal">
                                                <input type="number" id="itemTotal" name="itemTotal[]" class="form-control" readOnly=""/>
                                             </td>
                                             <td data-name="delete">
                                                <button name="delete" id="delete" class='btn btn-danger glyphicon glyphicon-remove row-remove'><span aria-hidden="true" >×</span></button>
                                             </td>
                                          </tr>
                                       </tbody>
                                    </table>
                              ...
                              <div class="row">
                                 ...
                                 <div class="col-md-2">
                                    Discount : <input type="text" id="finalDisc" name="finalDisc" class="form-control" readOnly="" />
                                 </div>
                                 <div class="col-md-3">
